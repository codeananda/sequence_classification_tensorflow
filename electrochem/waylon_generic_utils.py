...
def load_dataset_at(
    index, normalize_timeseries=False, verbose=True
) -> (np.array, np.array):
    assert index < len(TRAIN_FILES), (
        "Index invalid. Could not load dataset at %d" % index
    )
    if verbose:
...
def plot_dataset(
    ...
    if limit is None:
        ...
        if not plot_classwise:
            ...
            assert (
                limit == 1
            ), "If plotting classwise, limit must be 1 so as to ensure number of samples per class = 1"
    ...
    if not plot_classwise:
        ...
        classwise_X_train_attention_list = [
            np.asarray(x) for x in classwise_X_train_attention_list
        ]
    ...
    if not plot_classwise:
        ...
        classwise_X_test_attention_list = [
            np.asarray(x) for x in classwise_X_test_attention_list
        ]
    ...
    train_df = pd.DataFrame(X_train, index=range(X_train.shape[0]), columns=columns)

    test_df = pd.DataFrame(X_test, index=range(X_test.shape[0]), columns=columns)
    ...
    fig, axs = plt.subplots(
        rows, cols, squeeze=False, tight_layout=True, figsize=(8, 6)
    )
    ...
    if plot_data is not None and X_train_attention is not None:
        ...
        train_attention_df = pd.DataFrame(
            X_train_attention, index=range(X_train_attention.shape[0]), columns=columns
        )
        ...
        train_attention_df.plot(subplots=False, legend="best", ax=axs[1][0])

    if plot_data is not None and X_test_attention is not None:
        columns = ["Class %d" % (i + 1) for i in range(X_test_attention.shape[1])]
        test_df = pd.DataFrame(
            X_test_attention, index=range(X_test_attention.shape[0]), columns=columns
        )
        ...
        test_df.plot(subplots=False, legend="best", ax=axs[1][1])
...
def cutoff_choice(dataset_id, sequence_length):
    print(
        "Original sequence length was :",
        sequence_length,
        "New sequence Length will be : ",
...
def cutoff_sequence(X_train, X_test, choice, dataset_id, sequence_length):
    assert MAX_SEQUENCE_LENGTH_LIST[dataset_id] < sequence_length, (
        "If sequence is to be cut, max sequence"
