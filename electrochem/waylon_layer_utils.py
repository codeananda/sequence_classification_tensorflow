...
class AttentionLSTM(Recurrent):
    ...
    def get_constants(self, inputs, training=None):
        ...
        if self.implementation != 0 and 0 < self.dropout < 1:
            ...
            constants.append([K.cast_to_floatx(1.0) for _ in range(4)])
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            i = self.recurrent_activation(
                x_i
                + K.dot(h_tm1 * rec_dp_mask[0], self.recurrent_kernel_i)
                + K.dot(z_hat, self.attention_i)
            )
            f = self.recurrent_activation(
                x_f
                + K.dot(h_tm1 * rec_dp_mask[1], self.recurrent_kernel_f)
                + K.dot(z_hat, self.attention_f)
            )
            c = f * c_tm1 + i * self.activation(
                x_c
                + K.dot(h_tm1 * rec_dp_mask[2], self.recurrent_kernel_c)
                + K.dot(z_hat, self.attention_c)
            )
            o = self.recurrent_activation(
                x_o
                + K.dot(h_tm1 * rec_dp_mask[3], self.recurrent_kernel_o)
