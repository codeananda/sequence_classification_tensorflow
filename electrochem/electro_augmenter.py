...
class ElectroAugmenter(Sequence):
	...
	def __init__(
		...
		self.should_aug = (self.rng.random_sample(len(self.ordering)) < augmentation_percentage)
	...
	def __getitem__(self, idx):
		...
		for idx in idx_list:
			...
			if(self.should_aug[idx]):
				...
				aug_X = self._up_down_random_shift(aug_X, self.noise_shift, self.noise_shift_scale)
				# Don't augment Y
				aug_Y = base_Y
