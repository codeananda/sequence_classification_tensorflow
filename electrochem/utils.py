    ...
    config = edict(
        {
            ...
            "plotting": {"start_plotting_epoch": 0},
        }
    )
...
def load_scale_reshape_X_y(config):
    ...
    if config.model["model_type"].upper() == "LSTM":
        ...
        raise ValueError(
            """Please enter supported config.model['model_type']:
                            LSTM or Attention"""
        )
...
def scale_X_y(config, df_X, df_y):
    ...
    """
    df_X_scaled = scale_df_X_to_range(
        df_X, config.data_processing.a, config.data_processing.b
    )
    # No need to scale y
...
def _scale_to_range(seq, a, b, min=None, max=None):
    ...
    if min is None:
        min = min(seq)
    assert min < max
    scaled_seq = np.array([_scale_val(val, a, b, min, max) for val in seq])
...
def build_ATTENTION(config):
    ...
    model.compile(
        loss=config_model["loss"], optimizer=optimizer, metrics=config_model["metrics"]
    )
    return model
...
def get_callbacks(config, X_train, y_train):
    ...
    # EarlyStopping
    es = EarlyStopping(
        patience=config_callbacks["patience"],
...
def train(config):
    ...
    wandb_enable = config.get("wandb", True)
    if wandb_enable:
        wandb_run = wandb.init(project="electrochemistry", config=config)

    try:
        history = fit_model(config, model, X_train, X_val, y_train, y_val)
        # Plot loss, accuracy and f1_micro curves
        loss_fig = plot_metric(
            history, metric="loss", start_epoch=config.plotting["start_plotting_epoch"]
        )
        acc_fig = plot_metric(
            history,
            metric="accuracy",
            start_epoch=config.plotting["start_plotting_epoch"],
        )
        f1_fig = plot_metric(
            history,
            metric="f1_micro",
        ...
        if wandb_enable:
            fig_dict = {
                "Loss - Training and Validation": loss_fig,
