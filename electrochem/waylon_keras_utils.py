...
from waylon_generic_utils import (
    load_dataset_at,
...
from waylon_constants import MAX_SEQUENCE_LENGTH_LIST
...
def train_model(
    ...
    model_checkpoint = ModelCheckpoint(
        "./weights/{dataset_prefix}_{model_num}_weights.h5".format(
            dataset_prefix=dataset_prefix, model_num=model_num
        ),
        verbose=1,
        monitor="val_acc",
        save_best_only=True,
    ...
    if sequence_length != MAX_SEQUENCE_LENGTH_LIST[dataset_id]:
        ...
        if choice not in ["pre", "post"]:
            ...
            _, X_test = cutoff_sequence(
                None, X_test, choice, dataset_id, sequence_length
            )
...
def hyperparameter_search_over_model(
    model_gen, dataset_id, param_grid, cutoff=None, normalize_timeseries=False
):
    ...
    if sequence_length != MAX_SEQUENCE_LENGTH_LIST[dataset_id]:
        ...
        if choice not in ["pre", "post"]:
            ...
            X_train, _ = cutoff_sequence(
                X_train, None, choice, dataset_id, sequence_length
            )
    ...
    clf = KerasClassifier(
        build_fn=model_gen, epochs=50, class_weight=class_weight, verbose=0
    )
...
def build_function(model, layer_names=None, outputs=None):
    ...
    if layer_names is not None and (
        type(layer_names) != list and type(layer_names) != tuple
    ):
    ...
    if outputs is None:
        if layer_names is None:
            ...
            outputs = [
                layer.output for layer in model.layers if layer.name in layer_names
            ]
    else:
        outputs = outputs
...
def get_outputs(model, inputs, eval_functions, verbose=False):
    if verbose:
        print("----- activations -----")
    outputs = []
    layer_outputs = [func([inputs, 1.0])[0] for func in eval_functions]
    for layer_activations in layer_outputs:
        outputs.append(layer_activations)
    return outputs


def visualize_context_vector(
    model: Model,
    dataset_id,
    dataset_prefix,
    cutoff=None,
    ...
    eval_functions = build_function(
        model, attn_lstm_layer.name, outputs=[attention_output]
    )
    attention_vectors = []

    for i in range(X_train.shape[0]):
        activations = get_outputs(
            model, X_train[i, :, :][np.newaxis, ...], eval_functions, verbose=False
        )[0]
    ...
    if visualize_sequence:
        ...
        train_df = pd.DataFrame(
            {"attention (%)": attention_vector_final},
...
def visualize_cam(
    ...
    if sequence_length != MAX_SEQUENCE_LENGTH_LIST[dataset_id]:
        ...
        if choice not in ["pre", "post"]:
            ...
            X_train, _ = cutoff_sequence(
                X_train, _, choice, dataset_id, sequence_length
            )
    ...
    conv_cam_df = pd.DataFrame(conv_cam, index=range(conv_cam.shape[0]), columns=[1])
    ...
    sequence_df.plot(
        title="Sequence (class = %d)" % (class_label),
        subplots=False,
        legend=None,
        ax=axs[0][0],
    )
    ...
    conv_cam_df.plot(
        title="Convolution Class Activation Map (class = %d)" % (class_label),
        subplots=False,
        legend=None,
        ax=axs[1][0],
    )
